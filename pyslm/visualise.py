...
def plotPolygon(polygons: List[Any], zPos: Optional[float] = 0.0,
    ...
    for contour in contourCoords:
        ...
        if plot3D:
            ...
            if plotFilled:
                polygon = matplotlib.patches.Polygon(contour,
                                                     fill=True, linewidth=lineWidth,
                                                     edgecolor=lineColor, color=fillColor,
                                                     facecolor=fillColor)
...
def plotSequential(layer: Layer,
                   ...
                   handle: Optional[Tuple[plt.Figure, plt.Axes]] = None) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots sequentially the all the scan vectors (contours and hatches) for all :class:`LayerGeometry` in the
    provided :class:`Layer` using `Matplotlib`. The :class:`Layer` may be only plotted across a single layer.
...
def plot(layer: Layer,
    ...
    if plotPoints:
        ...
        if len(pointGeoms) > 0:
            ...
            if plot3D:
                ax.scatter3D(scatterPoints[:, 0], scatterPoints[:, 1], zPos, c=pntColors)
            else:
                ax.scatter(scatterPoints[:, 0], scatterPoints[:, 1], c=pntColors)
...
def plotHeatMap(part: Part,
    ...
    expPointTrans = np.floor(exposurePoints[:, :2] / resolution).astype(np.int64)

    # Get a bitmap object to work on
    bitmapSlice  = part.getBitmapSlice(z, resolution).astype(np.int64)
