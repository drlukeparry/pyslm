...
class BlockSupportBase(SupportStructure):
    ...
    overhang region, that may intersect with the Base Plate (:math:`z=0`) or self-intersect with :class:`Part`.
    These are generated externally in the :class:`BlockSupportGenerator` and other derived generator classes that
    build upon this. These objects represent the data structure for the support rather than the methods for
    generating them.

    The support volume (:attr:`~SupportStructure.supportVolume` is a generic 3D volume mesh that enables differentiation
    of support structures to be generated by creating a derived class that re-defines the abstract method
    :meth:`~SupportStructure.geometry`.
    """
...
class BaseSupportGenerator(abc.ABC):
    ...
    This class provides the base methods used for identifying some fundamental geometrical unsupported features
...
class BlockSupportGenerator(BaseSupportGenerator):
    ...
    After passing the geometry and setting the required parameters, the user requires calling
    :meth:`identifySupportRegions` to generate the support volumes.
    ...
    (:attr:`SupportStructure.intersectsPart`). Non-intersecting regions are connected to the build-plate only.
    ...
    with the originating part by adding Z-offsets (:attr:`lowerProjectionOffset` & :attr:`upperProjectedOffset`).

    Finally, these regions are intersected with the part using the Cork library to produce the final
    ...
    def minimumAreaThreshold(self) -> float:
        ...
        Support regions with a smaller area will not be generated.
        """
